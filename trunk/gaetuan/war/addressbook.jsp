...
<%
	...
	if (keyOffset != null)
		keyOffsetId = Long.decode(keyOffset);
	if (keyOffset == null)
		keyOffset = "";
...
<p>
<p>Select entries with Last name: <input name="lastNameSelected"
	value="<%=lastNameSelected%>"> and/or State: <input
	name="stateSelected" value="<%=stateSelected%>"> <input
	type="submit" value="Search"></p>
</form>

<%
	List<AddressBookEntry> entries = new ArrayList<AddressBookEntry>(
			AddressBookUtils.getPage(keyOffsetId, indexOffset,
					lastNameSelected, stateSelected));
...
<a href="/addressbook.jsp?page=<%=lastEntry.getId().toString()%>">Next
page by key offset</a>
<a href="/addressbook.jsp?startIndex=<%=indexOffset + 3%>">Next
page by index offset</a>
<%
	} else {
%>
...
<p>First name: <input name="firstName"> Last name: <input
	name="lastName"></p>
