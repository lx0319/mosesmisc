...
//              Logger.getLogger(HtmlHandlerHelper.class.getName());
        ...
        public static boolean isHtmlContentType(HttpURLConnection conn) {
                return conn.getContentType().trim().toLowerCase().startsWith("text/html");
        }
        ...
        public static boolean guessIsGZipCompressed(byte[] httpBuff) {
                ...
                if (httpBuff.length > 1) {
                        ...
                        gzipped = CommonUtils.comparebytes(
                                        CommonUtils.int2bytes(GZIPInputStream.GZIP_MAGIC),
                                        httpBuff, 0, 2);
                }
        ...
        public static String guessCharset(byte[] httpBuff) {
                ...
                String default_charset = "ISO-8859-1", charset = default_charset,
                        charsetToken = "charset=";
                
                try {

                        // This charset token finding is CASE SENSITIVE.
                        int charset_token_pos = CommonUtils.findBytes(
                                        httpBuff, charsetToken.getBytes(charset), 0);
                        
                        if (charset_token_pos == -1)
                                throw new Exception();
                        
                        int charset_value_start_pos = charset_token_pos + charsetToken.length();

                        if (CommonUtils.findByte(httpBuff,
                                        new byte[] {'\'', '\"'}, charset_value_start_pos, 1) == charset_value_start_pos)
                        ...
                        charset = new String(httpBuff,
                                        charset_value_start_pos,
                                        charset_value_end_pos - charset_value_start_pos,
                                        charset).trim();
        ...
        public static byte[] ungzip(byte[] gzippedBuff)
                throws IOException {
                ...
                return CommonUtils.readStreamToBuffer(
                                new GZIPInputStream(new ByteArrayInputStream(gzippedBuff)));
        }

        public static byte[] gzip(byte[] ungzippedBuff)
                throws IOException {

                ByteArrayOutputStream osBuff = new ByteArrayOutputStream();

                try {
                        CommonUtils.writeBufferToStream(
                                        new GZIPOutputStream(osBuff), ungzippedBuff);
